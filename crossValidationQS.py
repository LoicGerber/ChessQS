...
# server = 'localhost'
# server = 'tesla-k20c.gaia.unil.ch'  # Server address for g2s
server = 'mercury.gaia.unil.ch'  # Server address for g2s
...
metrics_max_lag   = 30  # Maximum lag for metrics computation
...
dirPath = '/path/to/saving/file.tif'
os.makedirs(dirPath, exist_ok=True)

print("Opening .h5 file...")
input = '/path/to/input/data.h5'
...
for entry in all_results:
    tiles[entry['tile_index']].append(entry['simulation_result'])

uncertainty_maps = {}

for tile_idx, sims in tiles.items():
    sims_arr = np.stack(sims, axis=0)  # shape (R,H,W)
    sims_arr_masked = sims_arr.copy()
    mask = (sims_arr_masked == -2)
    sims_arr_masked[mask] = np.nan
    classes = np.unique(sims_arr_masked[~np.isnan(sims_arr_masked)])
    valid_counts = np.sum(~np.isnan(sims_arr_masked), axis=0)   # denominator
    class_counts = np.stack([(sims_arr_masked == c).sum(axis=0) for c in classes], axis=0)
    class_prob   = class_counts / valid_counts
    # Shannon entropy per pixel
    # H = - Σ p * log(p)
    H = - np.nansum(class_prob * np.log(class_prob), axis=0)
    H_norm = H / np.log(len(classes))
    H_norm[mask[0,:,:]] = np.nan
    uncertainty_maps[tile_idx] = H_norm

for tile_idx, H_norm in uncertainty_maps.items():
    plt.figure()
    plt.imshow(H_norm, vmin=0, vmax=1, cmap='turbo')
    plt.colorbar(label="Shannon's Entropy")
    plt.title(f"Tile {tile_idx}")
    plt.axis("off")
    plt.ylim(350,650)
    plt.xlim(350,650)
    plt.savefig(f"{dirPath}/entropy_tile_{tile_idx:02d}.png", dpi=300, bbox_inches='tight')
    plt.show()
